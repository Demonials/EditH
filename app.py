    ...
    async def giveaway_countdown(self, giveaway_id, channel, end_time):
        try:
            ...
            if len(participants) < giveaway_data['winners']:
                await channel.send(f"❌ Not enough participants for **{giveaway_data['name']}**! (Need {giveaway_data['winners']}, got {len(participants)})")
                return
            ...
            logger.info(f"🎁 Giveaway completed: {giveaway_data['name']} with {len(winners)} winners")
